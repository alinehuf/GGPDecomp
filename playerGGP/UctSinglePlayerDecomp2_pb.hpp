...
class SubNode {
    ...
    bool fullyExplored(SetTermPtr legal); // toutes les actions légales sont totalement explorées
    bool allChildsFullyExplored(); // toutes les actions de toutes les transitions au départ d'un noeud sont totalement explorées
    int nbFullyExplored();
    ...
    const size_t depth;
    size_t visits;
    std::vector<SubLinkPtr> childs; // les transitions (chaque transition peut-être assciée à plusieurs actions)
    std::unordered_set<TermPtr> childs_moves; // toutes les actions associées à une des transitions 
    bool terminal;
...
//    void intersect(SetTermPtr& s1, const SetTermPtr& s2);
    ...
    void createGraphvizFile(const std::string& name, int s, bool verbose = true) const;
    ...
    std::vector<SubNodePtr> root; // racine de l'arbre
    VectorBool init_vals; // valeurs des entrées du circuit correspondantes à la racine
    VectorBool current; // position courante dans la descente
    std::vector<std::vector<SubNodePtr>> descent_node; // noeuds visités
    std::vector<std::vector<SubLinkPtr>> descent_link; // transitions visitées
    std::vector<TermPtr> descent_move; // nom des actions associées aux transitions
